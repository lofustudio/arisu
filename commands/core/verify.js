...
module.exports = {
    ...
    execute(interaction) {
        ...
        const hasMemberRole = db.get(`roles_${interaction.member.id}.member`);
        const hasNotVerifyRole = db.get(`roles_${interaction.member.id}.notVerified`);
        ...
        if (hasMemberRole && hasNotVerifyRole === true) {
            db.set(`roles_${interaction.member.id}.member`, false);
            db.set(`roles_${interaction.member.id}.notVerified`, true);
        }

        if (hasMemberRole === true) return interaction.reply({ content: 'You are already a member, you silly goose!', ephemeral: true });

        try {
            db.set(`roles_${interaction.member.id}.member`, true);
            db.set(`roles_${interaction.member.id}.notVerified`, false);
            ...
            return interaction.reply({ content: 'An unexpected error occured. A report has been sent to tyger, please try again later.', ephemeral: true }) && console.log(err);
        }
    }
}
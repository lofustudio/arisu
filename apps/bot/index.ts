...
//       if (err) {
//         axios.get("https://raw.githubusercontent.com/lofustudio/arisu/main/apps/bot/.env.example").then((res) => {
...
if ("TOKEN" in process.env === false) {
  console.error("Token not found. Please check your .env file and try again.");
  process.exit(1);
}

if ("DATABASE_URL" in process.env === false) {
  console.error("Database URL not found. Please check your .env file and try again.");
  process.exit(1);
}

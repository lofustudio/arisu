    ...
    if (err) {
        ...
        try {
            access(".env.example", constants.W_OK, (err) => {
                if (err) {
                    console.log("Installation is broken. Please redownload Cookie from https://github.com/lofustudio/cookie/releases");
            ...
            await questions.prompt([
                {
                    ...
                    async validate(value: string) {
                        ...
                        const result: string = client.login(value).then(() => {
                            return true;
                        }).catch(() => {
                ...
                },
                {
                    ...
                    async validate(value) {
                        ...
                        const result: string = client.connect().then(() => {
                            client.end();
                            return true;
                        }).catch((err: Error) => {
                ...
                writeFileSync(".env", Buffer.from(`TOKEN=${answers.token}\nDATABASE_URL=${answers.database_url}`));
            }).catch((err: Error) => {
                console.log(err);
                process.exit(1);
            });
            ...
            const child = new (forever.Monitor)(process.env.NODE_ENV === "development" ? "./src/index.ts" : "./build/index.js", {
                args: [],
                max: 1,
            });
